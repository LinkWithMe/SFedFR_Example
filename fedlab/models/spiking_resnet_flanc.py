...
def make_model(args, parent=False):
    return spiking_ResNet18_FLANC(args, pretrained=False, spiking_neuron=neuron.LIFNode, surrogate_function=surrogate.ATan(), detach_reset=True)
...
class DecomBlock(nn.Module):
    def __init__(self, filter_bank, in_channels, out_channels, n_basis, basis_size, kernel_size, stride=1, bias=False, conv=conv3x3, spiking_neuron: callable = None, **kwargs ):
...
class spiking_ResNet18_FLANC(SpikingResNet):
    def __init__(self, args, conv3x3=conv3x3, conv1x1=conv1x1, spiking_neuron: callable = None,
        ...
        super(spiking_ResNet18_FLANC, self).__init__(T = 4, block = BasicBlock, layers = [2, 2, 2, 2], n_basis=0.25, basis_fract=0.125, net_fract= 1, conv3x3=conv3x3, conv1x1=conv1x1, zero_init_residual=False, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None,
        spiking_neuron=spiking_neuron)

        pretrained = False
        if conv3x3 == conv3x3:
            if pretrained:
                self.load(args, smnistrict=True)
