    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            with torch.no_grad():
                ...
                firerate_last = fr_monitor.records[-1] #获取最后一层的点火率之和
                # print("最后一层的点火率为：",firerate_last)
...
def evaluate_FireRate_front(model, criterion, test_loader):
    """Evaluate classify task model accuracy.
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            with torch.no_grad():
                ...
                firerate = torch.sum(fr_monitor.records[0]) #获取第一层的点火率之和
                firerate_.update(firerate.item())
                functional.reset_net(model)
                fr_monitor.remove_hooks()
...
def evaluate_FireRate_middle(model, criterion, test_loader):
    """Evaluate classify task model accuracy.
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            with torch.no_grad():
                ...
                # # 最后两层点火率之和
                last_two_records = torch.cat(fr_monitor.records[1:3])
...
def evaluate_FireRate_last(model, criterion, test_loader):
    """Evaluate classify task model accuracy.
